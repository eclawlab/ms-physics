"use strict";
// Copyright 2025-2026, University of Colorado Boulder
Object.defineProperty(exports, "__esModule", { value: true });
exports.toFixed = toFixed;
/**
 * A predictable implementation of toFixed.
 *
 * JavaScript's toFixed is notoriously buggy, behavior differs depending on browser,
 * because the spec doesn't specify whether to round or floor.
 * Rounding is symmetric for positive and negative values, see roundSymmetric.
 *
 * @author Chris Malley (cmalley@pixelzoom.com)
 * @author Michael Kauzmann (PhET Interactive Simulations)
 */
var dot_js_1 = require("../dot.js");
// Use ignore for ts 5.9 and 7.0 compatibility
// eslint-disable-next-line @typescript-eslint/prefer-ts-expect-error
// @ts-ignore - Big import requires this for some reason, but it works
var big_6_2_1_js_1 = require("../../../sherpa/lib/big-6.2.1.js"); // eslint-disable-line phet/default-import-match-filename
function toFixed(value, decimalPlaces) {
    assert && assert(Number.isInteger(decimalPlaces), "decimal places must be an integer: ".concat(decimalPlaces));
    if (isNaN(value)) {
        return 'NaN';
    }
    else if (value === Number.POSITIVE_INFINITY) {
        return 'Infinity';
    }
    else if (value === Number.NEGATIVE_INFINITY) {
        return '-Infinity';
    }
    // eslint-disable-next-line phet/bad-sim-text
    var result = new big_6_2_1_js_1.default(value).toFixed(decimalPlaces);
    // Avoid reporting -0.000
    if (result.startsWith('-0.') && Number.parseFloat(result) === 0) {
        return '0' + result.slice(2);
    }
    else {
        return result;
    }
}
dot_js_1.default.register('toFixed', toFixed);
