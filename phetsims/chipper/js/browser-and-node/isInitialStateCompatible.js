...
function areCompatible(groundTruthValue, testValue) {
    ...
    if (Array.isArray(groundTruthValue)) {
        ...
        for (var i = 0; i < groundTruthValue.length; i++) {
            var newItem = groundTruthValue[i];
            var oldItem = testValue[i];
    ...
    if (typeof groundTruthValue === 'object' && groundTruthValue !== null) {
        ...
        for (var key in groundTruthValue) {
 ...
 */
var isInitialStateCompatible = function (groundTruthState, testState) {
    return areCompatible(groundTruthState, testState);
};
exports.default = isInitialStateCompatible;
